...
merjou <- merge(dados_sinconv_cg, dados_geopb_cg, all=T)

merjou <- merjou %>% mutate(OBJETO_PROPOSTA = as.character(OBJETO_PROPOSTA)) %>% group_by(id.y)
merjou <- merjou %>% mutate(OBJETO_PROPOSTA = tolower(OBJETO_PROPOSTA), descricao_sucinta_obra = tolower(descricao_sucinta_obra))
merjou <- merjou %>% mutate(palavras_iguais = length(intersect(filtraArtigos(unlist(strsplit(OBJETO_PROPOSTA, " "))), filtraArtigos(unlist(strsplit(descricao_sucinta_obra, " "))))))
View(merjou %>% select(OBJETO_PROPOSTA, descricao_sucinta_obra, palavras_iguais))

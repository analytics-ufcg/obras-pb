...
p <- plot_ly(evolucao_tipos_a, x =~perc , y = ~nome, color = ~nome, type = 'bar', orientation = 'h', 
             marker = list(
                       color = cores)
        ) %>% layout(showlegend = FALSE,         xaxis = list(title = "% Total de Investimento (Desde 2012)"),
         yaxis = list(title ="")) 
...
p <- plot_ly(evolucao_tipos, x =~ano, y = ~custo_total, color = ~nome, type = 'bar', orientation = 'v',colors = "Reds") %>%

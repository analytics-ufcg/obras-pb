...
```{r}
custos_anuais <- obras.completo %>% filter(!is.na(data_cadastro), data_cadastro >= min_date) %>% 
    mutate(ano = format(data_inicio_obra,"%Y")) %>%  left_join(obra_georef, by = c("id" = "fk_obra"), x.all = TRUE) %>%
    group_by(ano) %>%
    summarise(n_obras = n(),
              n_georeferenciadas = sum(tem_georeferenciamento, na.rm = TRUE),
              n_nao_georeferenciadas = n() - n_georeferenciadas,
              custo_total = sum(valor_obra)) 

custos_anuais <- custos_anuais %>% mutate(ano = paste(ano,"-1-1",sep = "",collapse = NULL)) %>% 
    mutate(ano = as.Date(ano,"%Y-%m-%d"))

custos_anuais_xts <- xts(select(custos_anuais, 5), custos_anuais$ano)

min_zoom_date <- as.Date("2012-01-01", "%Y-%m-%d")

dygraph(custos_anuais_xts) %>%
    dySeries("custo_total", label = "Total Gasto", color = '#f05542') %>%
    dyAxis("x", label = "Ano") %>%

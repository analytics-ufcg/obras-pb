...
tabela_gastos_municipio <- tabela_gastos_municipio %>% mutate(custo_total = custo_total * 1000000)
...
ggplot(evolucao_tipos_a,aes(x= reorder(nome, perc), y=perc, fill = nome),legend.position="none") + geom_bar(stat='identity') + guides(fill=FALSE) + coord_flip() + xlab("") + ylab("Percentual")



```

### Custos Anuais Por Tipo de Obra (Em Milhões)

```{r}
min_zoom_date <- as.Date("2013-06-01", "%Y-%m-%d")
max_zoom_date <- as.Date("2018-07-01", "%Y-%m-%d")

evolucao_tipos <- evolucao_tipos %>% mutate(custo_total = custo_total / 1000000)

ggplot(evolucao_tipos ,aes(x= ano, y=custo_total, fill = nome),legend.position="none") + geom_bar(stat='identity', position=position_dodge()) +
  xlab("Período de Tempo") + ylab("Custo") + guides(fill=FALSE)
...
tipo.ranking <- obras.completo %>%
        filter(!is.na(data_cadastro), data_cadastro >= min_date) %>%
        collect(n = Inf) %>%
        left_join(tipos.das.obras, by = c("fk_tipo_obra" = "id")) %>% left_join(localidade, by = c("fk_localidade" = "id")) %>% group_by(nome.y,nome.x) %>% summarise(`Obras cadastradas` = n(),`Custo Unitário Médio` = mean(valor_obra)/mean(dimensao) ,`Custo Médio` = mean(valor_obra),  `Unidade de Medida`=  unique(unidadeMedida) ,`Custo Total` = sum(valor_obra)) %>% arrange(nome.x, desc(`Custo Unitário Médio`))

colnames(tipo.ranking) <- c("Município / Estado", "Tipo de Obra", "Obras Cadastradas", "Custo Unitário Médio", "Custo Médio","Unidade de Medida", "Custo Total")

datatable(tipo.ranking, extensions = c('Buttons', 'Responsive'),
          options = list(
              dom = 'Bfrtip', buttons = c('csv', 'excel', 'print')
              )) %>% formatCurrency(4,"R$",mark = ".", dec.mark = ",") %>% formatCurrency(7,"R$",mark = ".", dec.mark = ",") %>% formatCurrency(5,"R$",mark = ".", dec.mark = ",")

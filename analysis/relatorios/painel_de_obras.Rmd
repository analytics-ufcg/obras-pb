...
acompanhamento_mensal_xts <- xts(select(acompanhamento_mensal, 2), acompanhamento_mensal$mes_ano)

dygraph(acompanhamento_mensal_xts) %>%
    dySeries("n_obras", label = "Registros") %>%
    dyAxis("x", label = "Mês de cadastro") %>%
    dyAxis("y", label = "Quantidade de registros de acompanhamento") %>%
    dyOptions(useDataTimezone = TRUE, fillGraph = TRUE, plotter = 

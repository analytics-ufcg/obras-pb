...
obra.georref.centroide.sumarizado <- 
    obra.georref.corrigido %>%
    group_by(fk_obra) %>%
    mutate(lat = mean(lat), lon = mean(lon)) %>%
    select(-valor_georeferenciamento)

obra.georref.centroide.sumarizado <- obra.georref.centroide.sumarizado[!duplicated(obra.georref.centroide.sumarizado$fk_obra),]
...
obras.2013 <- obras.2013 %>% 
    left_join(municipios, by = "codigo_ibge") %>% 
    left_join(obra.georref.centroide.sumarizado %>% select(id,lat,lon), by = "id") %>% 
    mutate(
...
obras.filtradas.dimensao <- obras.2013 %>% 
    filter(dimensao > 0)
```
...
colors <- colorNumeric("YlOrRd", domain = c(min(mapa_paraiba_quantidade_obras@data$qtde.obras, na.rm = T), max(mapa_paraiba_quantidade_obras@data$qtde.obras, na.rm = T)))
...
mapa_paraiba_quantidade_obras %>% 
    ...
    addPolygons(opacity = 0.5, 
              weight = 1, 
              fillColor = colors(mapa_paraiba_quantidade_obras@data$qtde.obras),
              color = 'black', 
              label = mapa_paraiba_quantidade_obras@data$Nome_Munic,
              popup = paste('Município: ', mapa_paraiba_quantidade_obras@data$Nome_Munic, '</br>Custo mediano das obras: R$ ', format(mapa_paraiba_quantidade_obras@data$valor.mediano, big.mark=".", decimal.mark = ",", nsmall = 2, scientific = FALSE), '</br>Número de obras: ', mapa_paraiba_quantidade_obras@data$qtde.obras),
              fillOpacity = 1) %>%
    addLegend(position = "bottomright", pal = colors, values = mapa_paraiba_quantidade_obras@data$qtde.obras,
            title = "Quantidade de obras por município",
            opacity = 1)
```
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
gastos.por.habitante.por.municipio <- obras.filtradas.valor %>% 
    ...
    left_join(municipios.pb %>% rename("codigo_ibge" = "Código", "populacao" = "População.estimada...pessoas") %>% select(codigo_ibge, populacao),
              by = "codigo_ibge") %>% 
    mutate(gastos.por.habitante = valor.total.obras / populacao)

mapa_paraiba_gasto_habitante <- mapa_paraiba

mapa_paraiba_gasto_habitante@data <- mapa_paraiba_gasto_habitante@data %>%
  left_join(gastos.por.habitante.por.municipio,
            by = c("GEOCODIG_M" = "codigo_ibge"))

colors <- colorNumeric("YlOrRd", domain = c(min(mapa_paraiba_gasto_habitante@data$gastos.por.habitante, na.rm = T), max(mapa_paraiba_gasto_habitante@data$gastos.por.habitante, na.rm = T)))
...
mapa_paraiba_gasto_habitante %>% 
    ...
    addPolygons(opacity = 0.5, 
              weight = 1, 
              fillColor = colors(mapa_paraiba_gasto_habitante@data$gastos.por.habitante),
              color = "black", 
              label = mapa_paraiba_gasto_habitante@data$Nome_Munic,
              popup = paste("Município: ", mapa_paraiba_gasto_habitante@data$Nome_Munic, "</br>Valor em obras por habitante: R$", format(mapa_paraiba_gasto_habitante@data$gastos.por.habitante, big.mark=".", decimal.mark = ",", nsmall = 2, scientific = FALSE), "</br>Valor total das obras: R$", format(mapa_paraiba_gasto_habitante@data$valor.total.obras, big.mark=".", decimal.mark = ",", nsmall = 2, scientific = FALSE), "</br>Quantidade de habitante: ", mapa_paraiba_gasto_habitante@data$populacao),
              fillOpacity = 1) %>%
    addLegend(position = "bottomright", pal = colors, values = mapa_paraiba_gasto_habitante@data$gastos.por.habitante,
            title = "Valor gasto em obras por habitante",
            opacity = 1)
```
...
obra.georref.centroide.sumarizado %>% 
    ...
    mutate(
        label = str_to_upper(
            ifelse(nchar(descricao_sucinta_obra) <= 30,
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
municipior.nao.georref.prop <- obras.2013 %>% 
...
mapa_paraiba_nao_georreferenciada@data <- mapa_paraiba_nao_georreferenciada@data %>%
  left_join(municipior.nao.georref.prop,
            by = c("GEOCODIG_M" = "codigo_ibge"))

colors <- colorNumeric("YlOrRd", 
                       domain = c(
                           min(mapa_paraiba_nao_georreferenciada@data$prop.nao.georref, na.rm = T),
                           max(mapa_paraiba_nao_georreferenciada@data$prop.nao.georref, na.rm = T)
                           )
                       )
...
obras.2013.pavimentacao <-
    ...
    group_by(nome) %>%
    summarise(custo_efetivo = median(custo_efetivo))
```
...
colors <- colorNumeric("YlOrRd", domain = c(min(mapa_paraiba_custo_efetivo@data$custo_efetivo, na.rm = T), max(mapa_paraiba_custo_efetivo@data$custo_efetivo, na.rm = T)))
...
mapa_paraiba_custo_efetivo %>% 
    ...
    addPolygons(opacity = 0.5, 
              weight = 1, 
              fillColor = colors(mapa_paraiba_custo_efetivo@data$custo_efetivo),
              color = 'black', 
              label = mapa_paraiba_custo_efetivo@data$Nome_Munic,
              popup = paste('Município: ', mapa_paraiba_custo_efetivo@data$Nome_Munic, '</br>Custo efetivo: R$', format(mapa_paraiba_custo_efetivo@data$custo_efetivo, digits = 2, big.mark=".", decimal.mark = ",", scientific = FALSE)),
              fillOpacity = 1) %>%
    addLegend(position = "bottomright", pal = colors, values = mapa_paraiba_custo_efetivo@data$custo_efetivo,
            title = "Custo efetivo mediano das obras",
            opacity = 1)
```

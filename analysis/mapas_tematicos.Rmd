...
Nesta análise vamos identificar possíveis mapas temáticos para incorporar futuramente ao GeoPB, utilizando dados georeferenciados de obras da Paraíba. Para fazê-la, utilizaremos apenas dados de obras a partir de 2013. Quando utilizar o valor da obra, consideraremos apenas valores acima de R$ 1000,00 e quando utilizar a dimensão, consideraremos apenas obras com dimensão acima de 50 da respectiva unidade.
...
valor.obras.municipio <- obras.2013 %>% 
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
colors <- colorNumeric("viridis", domain = c(min(mapa_paraiba@data$qtde.obras, na.rm = T), max(mapa_paraiba@data$qtde.obras, na.rm = T)))
...
mapa_paraiba %>% 
    ...
    addPolygons(opacity = 0.5, 
              weight = 1, 
              fillColor = colors(mapa_paraiba@data$qtde.obras),
              color = 'black', 
              label = mapa_paraiba@data$Nome_Munic,
              popup = paste('Município: ', mapa_paraiba@data$Nome_Munic, '</br>Custo mediano das obras: R$ ', format(mapa_paraiba@data$valor.mediano, big.mark=".", decimal.mark = ",", nsmall = 2, scientific = FALSE), '</br>Número de obras: ', mapa_paraiba@data$qtde.obras),
              fillOpacity = 1) %>%
    addLegend(position = "bottomright", pal = colors, values = mapa_paraiba@data$qtde.obras,
...
colors <- colorNumeric("viridis", domain = c(min(mapa_paraiba@data$gastos.por.habitante, na.rm = T), max(mapa_paraiba@data$gastos.por.habitante, na.rm = T)))
...
mapa_paraiba %>% 
    ...
    addPolygons(opacity = 0.5, 
              weight = 1, 
              fillColor = colors(mapa_paraiba@data$gastos.por.habitante),
              color = "black", 
              label = mapa_paraiba@data$Nome_Munic,
              popup = paste("Município: ", mapa_paraiba@data$Nome_Munic, "</br>Valor em obras por habitante: R$", format(mapa_paraiba@data$gastos.por.habitante, big.mark=".", decimal.mark = ",", nsmall = 2, scientific = FALSE), "</br>Valor total das obras: R$", format(mapa_paraiba@data$valor.total.obras, big.mark=".", decimal.mark = ",", nsmall = 2, scientific = FALSE), "</br>Quantidade de habitante: ", mapa_paraiba@data$populacao),
              fillOpacity = 1) %>%
    addLegend(position = "bottomright", pal = colors, values = mapa_paraiba@data$gastos.por.habitante,
...
## Obras georeferenciadas 2013-2017

Vejamos agora o mapa de pontos das obras georeferenciadas, onde os detalhes podem ser vistos clicando na obra:
...
E agora os pontos georeferenciados, substituindo a localização pelo centro da cidade quando o dado é inexistente, onde os detalhes podem ser vistos clicando na obra:
...
Agora vamos dar uma olhada no custo efetivo das obras.

Selecionando apenas as obras a partir de 2013 e com valor maior que 1000 reais.
...
Vamos ver agora os cinco tipos de obra mais executadas, considerando apenas obras com dimensão maior que zero.
...
Agora vamos selecionar apenas o tipo mais frequente (PAVIMENTAÇÃO PARALEPÍPEDO) e com dimensão maior do que 50m2.
...
colors <- colorNumeric("viridis", domain = c(min(mapa_paraiba@data$custo_efetivo, na.rm = T), max(mapa_paraiba@data$custo_efetivo, na.rm = T)))
...
mapa_paraiba %>% 
    ...
    addPolygons(opacity = 0.5, 
              weight = 1, 
              fillColor = colors(mapa_paraiba@data$custo_efetivo),
              color = 'black', 
              label = mapa_paraiba@data$Nome_Munic,
              popup = paste('Município: ', mapa_paraiba@data$Nome_Munic, '</br>Custo efetivo: R$', format(mapa_paraiba@data$custo_efetivo, digits = 2, big.mark=".", decimal.mark = ",", scientific = FALSE)),
              fillOpacity = 1) %>%
    addLegend(position = "bottomright", pal = colors, values = mapa_paraiba@data$custo_efetivo,

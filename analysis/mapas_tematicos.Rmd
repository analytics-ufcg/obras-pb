...
Selecionando apenas obras a partir de 2013 e com valor maior que 1000 reais.
...
Vamos ver agora qual o tipo de obra mais executada, considerando apenas observações com dimensão maior que zero.
...
Selecionando apenas o tipo mais frequente (PAVIMENTAÇÃO PARALEPÍPEDO) e com dimensão maior do que 50m2.
...
obras.2013.escola <-
    obras.2013 %>%
    filter(tipo_obra == 'ESCOLA' & dimensao > 50) %>%
    select(valor_obra, dimensao, nome, descricao_sucinta_obra) %>%
    mutate(custo_efetivo = valor_obra/dimensao)
...
mapa_paraiba %>% 
    ...
    addPolygons(opacity = 0.5, 
              ...
              color = 'black', 
              label = mapa_paraiba@data$Nome_Munic,
              popup = paste('Município: ', mapa_paraiba@data$Nome_Munic, '</br>Custo efetivo: R$', mapa_paraiba@data$custo_efetivo),
              fillOpacity = 1) %>%

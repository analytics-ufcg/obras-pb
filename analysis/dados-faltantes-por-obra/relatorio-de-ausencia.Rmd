...
obra.medicao.agrupada <- obra.medicao %>% 
    ...
    summarise(total_de_medicoes = n(), 
              ...
              data.medicao.final = max(data_final, na.rm = TRUE)) %>% 
    left_join(situacao.medicao.por.data, by = c("fk_obra", "data.medicao.final" = "data_final"))
...
* Com informações em andamento (deve satisfazer pelo menos um dos critérios):
    ...
    + Se a data do último acompanhamento não exceder 90 dias
...
tb.dados.municipios <- tb.dados.municipios %>% 
    mutate(
        ...
        tempo.medicao.final = difftime(lubridate::ymd(ultima.data), lubridate::ymd(data.medicao.final),units="days"),
        acomp.final.menor.90.dias = !is.na(tempo.acomp.final) & tempo.acomp.final <= 90,
        medicao.final.menor.90.dias = !is.na(tempo.medicao.final) & tempo.medicao.final <= 90,
        situacao_estimada = ifelse(!is.na(situacao) & situacao %in% c(7, 8) | !is.na(data_recebimento_obra),
                          "COM INFORMAÇÕES DE CONCLUÍDA",
                          ifelse(!is.na(situacao) & situacao == 1 & (medicao.final.menor.90.dias | acomp.final.menor.90.dias | tempo.de.obra <= 180), 
    ...
    select(-tempo.acomp.final, -tempo.medicao.final, -acomp.final.menor.90.dias, -medicao.final.menor.90.dias)
```
...
* numero_art:
    ...
    + VÁLIDO: ARTS que seguirem o padrão: UF (PB) + ANO (2014-2018) + 7 dígitos numéricos e tiverem data a partir de 2015
...
obras.metricas.relevantes <- tb.dados.municipios %>% 
    mutate(
        ...
        corretude_numero_art = ifelse(valor_obra >= 10000 & valor_obra < 30000, "OPCIONAL",
                                      ifelse(str_detect(numero_art, paste0("(P|p)(B|b)\\s?(201[4-8]|1900)([:digit:]{7})")) & 
                                                 lubridate::year(data_inicio_obra) >= 2015, "VALIDO", 
        ...
        corretude_valor_obra = ifelse(valor_obra <= 0 | is.na(valor_obra), "INVALIDO",
                                      ifelse(valor_obra >= 10000 & valor_obra < 100000000, "VALIDO", "INCERTO")),
        dimensao_valida = !is.na(dimensao) & dimensao > 0,
        corretude_dimensao = ifelse(!is.na(dimensao) & dimensao > 0, "VALIDO", "INVALIDO"),
        tipo_obra_valida = !is.na(tipo_obra) & tipo_obra != "OUTRAS",
        corretude_tipo_obra = ifelse(is.na(tipo_obra), "INVALIDO",
                                     ifelse(tipo_obra != "OUTRAS", "VALIDO",
                                            "INCERTO")),
        corretude_tem_georreferenciamento = ifelse(valor_obra >= 10000 & valor_obra < 30000, "OPCIONAL",
        ...
        corretude_dentro_municipio = ifelse(valor_obra >= 10000 & valor_obra < 30000, "OPCIONAL",
        ...
        corretude_planilha_contratada = ifelse(valor_obra >= 10000 & valor_obra < 30000, "OPCIONAL",
        ...
        corretude_numero_contrato = ifelse(is.na(numero_contrato) | numero_contrato == "0000" | numero_contrato == "-", "INVALIDO", "VALIDO"),
        ...
        corretude_total_de_medicoes = ifelse(valor_obra >= 10000 & valor_obra < 150000, "OPCIONAL",
        ...
        corretude_medicoes_ok = ifelse(valor_obra >= 10000 & valor_obra < 150000, "OPCIONAL",
        ...
        corretude_num_fotos_medicao = ifelse(valor_obra >= 10000 & valor_obra < 150000, "OPCIONAL",
        ...
        corretude_num_fotos_acompanhamento = ifelse(valor_obra >= 10000 & valor_obra < 30000, "OPCIONAL",
                                                    ifelse(num_fotos_acompanhamento > 0, "VALIDO",
                                                           ifelse(tempo.de.obra <= 90 | valor_obra < 10000, "INCERTO", "INVALIDO"))),
        data_inicio_obra_valida = !is.na(data_inicio_obra),
        corretude_data_inicio_obra = ifelse(!is.na(data_inicio_obra), "VALIDO", "INVALIDO"),
        data_prevista_conclusao_valida = !is.na(data_prevista_conclusao),
        corretude_data_prevista_conclusao = ifelse(!is.na(data_prevista_conclusao), "VALIDO", "INVALIDO"),
        data_conclusao_obra_valida = !is.na(data_conclusao_obra) | situacao_estimada == "EM ANDAMENTO",
        corretude_data_conclusao_obra = ifelse(!is.na(data_conclusao_obra) | situacao_estimada == "COM INFORMAÇÕES EM ANDAMENTO" |
        ...
        corretude_data_recebimento_obra = ifelse(!is.na(data_recebimento_obra) | situacao_estimada == "COM INFORMAÇÕES EM ANDAMENTO" |
                                                   situacao_estimada == "COM INFORMAÇÕES DE NÃO CONCLUÍDA", "VALIDO",
                                               ifelse(is.na(data_recebimento_obra) & situacao_estimada == "COM INFORMAÇÕES DE CONCLUÍDA" |
                                                          data_recebimento_obra < data_conclusao_obra, "INVALIDO", "INCERTO")),
        ...
        corretude_projeto_basico = ifelse(valor_obra >= 10000 & valor_obra < 30000, "OPCIONAL",
                                          ifelse(!is.na(projeto_basico), "VALIDO",
                                                 ifelse(valor_obra < 10000, "INCERTO", "INVALIDO"))),
        corretude_tem_acompanhamento_inicial = ifelse(valor_obra >= 10000 & valor_obra < 30000, "OPCIONAL",
        ...
        corretude_tem_acompanhamento_final = ifelse(valor_obra >= 10000 & valor_obra < 30000, "OPCIONAL",
        ...
        corretude_numero_obra = ifelse(esfera != 1, "OPCIONAL",
        ...
        corretude_situacao_estimada = ifelse(situacao_estimada == "DESATUALIZADA", "INVALIDO", "VALIDO")
    ) %>% 
    select(
        municipio,
        descricao_sucinta_obra,
        ...
        obras.filt.mun %>%
            formattable(list(
              ...
              situacao_estimada = formatter(
                "span",
                class = ~ paste("celula", ifelse(situacao_estimada_valida, "green", "red")))
          )) %>%
          as.datatable(
              options = list(
                autoWidth = TRUE,

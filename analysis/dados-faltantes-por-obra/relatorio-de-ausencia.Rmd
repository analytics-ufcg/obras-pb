...
obras.metricas.relevantes <- tb.dados.municipios %>% 
    mutate(
        ...
        numero_art_valido = str_detect(numero_art, paste0("(P|p)(B|b)(",lubridate::year(data_inicio_obra), "|1900)([:digit:]{7})")) | 
            valor_obra <= 30000 | lubridate::year(data_inicio_obra) < 2015,
...
A mediana de critérios válidos por município é cerca de 52% e os mesmos tem pequena variação, visto que 50% dos municípios preenchem entre cerca de 47% e 57% dos critérios. Apesar da baixa variação, existem municípios que se destacaram positivamente, tendo cerca de 72% dos critérios preenchidos.
...
Vemos que os municípios que mais se destacaram foram Areia e São João do Tigre, os quais tem pouco mais de 70% dos critérios preenchidos corretamente.

...
* tipo_obra - os que tiverem valores iguais a "OUTRAS" ou inexistente
...
df <- obras.metricas.relevantes %>% 
    mutate(grupo = 1) %>% 
    group_by(grupo) %>% 
    summarise(tipo_obra = sum(tipo_obra_valida) / n(),
              ...
              num_fotos = sum(num_fotos_valida) / n(), 
...
df %>% 
    ggplot(aes(x = reorder(colunas, porcentagem.criterios.validos), y = porcentagem.criterios.validos * 100)) +
    geom_bar(stat = "identity") +
    coord_flip() +
    labs(x = "Porcentagem de dados válidos", y = "Critérios", title = "Porcentagem dos critérios válidos")
```
...
obras.metricas.relevantes <- obras.metricas.relevantes %>% 
    ...
    mutate(
        dados_validos = sum(tipo_obra_valida, valor_obra_valida, dimensao_valida, dentro_municipio_valido, 
                            ...
                            num_fotos_valida, tem_georreferenciamento_valido, numero_art_valido)
        )

panorama.municipios <- obras.metricas.relevantes %>% 
    group_by(municipio) %>% 
    summarise(porc.dados.validos = sum(dados_validos) / (15 * n())) %>% 
    arrange(-porc.dados.validos)

panorama.municipios %>% 
    ggplot(aes(x = "municipios", y = porc.dados.validos * 100)) + 
    geom_boxplot() + 
    labs(x = "Municípios", y = "Porcentagem de critérios válidos", title = "Porcentagem de critérios válidos por município")
```

A mediana de critérios válidos por município é cerca de 35% e os mesmos tem pequena variação, visto que 50% dos municípios preenchem entre cerca de 32% e 38% dos critérios. Apesar da baixa variação, existem municípios que se destacaram positivamente, tendo mais de 50% dos critérios preenchidos.

Vejamos os municípios mais detalhadamente:

```{r, echo=FALSE, warning=FALSE, fig.height=25}
panorama.municipios %>% 
...
div(bsselect(municipios.pb$Nome.do.Município, type = "text"))
...
tags$script('
    $(document).ready(function(){
    		...
    		$(".selectpicker").change(function(){
        		...
        		var selector1 = ".table-municipio:not(:contains(" + selected + ":))"
        		var selector2 = ".table-municipio:contains(" + selected + ":)"
        		$(selector1).parent().css("display", "none")
        		$(selector2).parent().css("display", "")
            })
    })
')
...
show_plot <- function(mun, porc.dados.validos, plot_object) {
    div(style="margin:auto;text-align:center",
        div(class="table-municipio",
            h3(paste0(mun, ": ", format(porc.dados.validos, decimal.mark=",", digits=2, nsmall=2), "% dos critérios válidos"))),
...
do.call(div, lapply(municipios, function(mun) {
    obras.filt.mun <- obras.metricas.relevantes %>%
            ...
            arrange(-dados_validos, -numero_art_valido, -valor_obra_valida, -dimensao_valida, -tipo_obra_valida,
                  ...
                  -total_de_medicoes_valida, -medicoes_ok_valida, -num_fotos_valida, -data_inicio_obra_valida,
                  -data_prevista_conclusao_valida, -data_conclusao_obra_valida, -data_recebimento_obra_valida)

    porc.dados.validos <- sum(pull(obras.filt.mun, dados_validos)) / (15 * nrow(obras.filt.mun)) * 100

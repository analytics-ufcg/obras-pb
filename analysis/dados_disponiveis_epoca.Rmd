...
Como já vimos anteriormente, a maioria das obras está em andamento, mais especificamente 97.5% das obras cadastradas no sistema. 
...
No gráfico abaixo veremos quais orgãos públicos possuem menos obras em andamento, talvez fosse desejável ver quais orgãos possuem MAIS obras em andamento para verificar aqueles que não se preocupam em mudar o status das obras, contudo, estes seriam a esmagadora maioria uma vez que temos algo em torno de 90% dos orgãos com 100% de obras em execução. Vejamos o gráfico:

```{r}

obra.evolucao.jurisdicionado <- obra %>% 
    left_join(evolucao, by = c("id" = "fk_obra")) %>% 
    select(id, data_inicio_obra, valor_obra, descricao_sucinta_obra, fk_jurisdicionado, andamento) %>% 
    mutate(ano = lubridate::year(data_inicio_obra)) %>%
    left_join(jurisdicionado_db2, by = c("fk_jurisdicionado" = "id"))
...
orgaos_menos_obras_em_andamento %>%
    ggplot(aes(x = reorder(nome, -porcentagem), y = porcentagem)) +
    geom_col() + 
    coord_flip() +

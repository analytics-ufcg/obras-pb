...
obras.filtradas %>% 
    ggplot(aes(x = "ano", y = ano)) +
        ...
        labs(x = "ano",
             ...
             subtitle = "Considerando as obras a partir de 2003")
```

Vemos também que metade das obras foram construídas antes de 2008 e 25% das obras são datadas a partir de 2012. 
Das obras que iniciaram a partir de 2003 todas possuem o município em que foram construída, além de possuir também a microregião e mesoregião que pertencem.
...
geolocalizacao.temporal %>%     
    ...
    ggplot(aes(x = ano, y = porcentagem.localizacao.disponivel)) +
        ...
        labs(y = "porcentagem das obras georeferenciadas disponíveis", 
             ...
             subtitle = "Considerando obras a partir de 2003")
```
...
geolocalizacao.temporal %>%     
    ...
    ggplot(aes(x = ano, y = obras.geolocalizadas)) +
        ...
        labs(y = "quantidade de obras georeferenciadas", 
             ...
             subtitle = "Considerando obras depois de 2003")
```
...
obras.agrup.tipo.geo %>% 
    ggplot(aes(x = reorder(tipo_georeferenciamento, -proporcao), y = proporcao)) +
...
tipo.obra %>% 
    ...
    ggplot(aes(x = ano, y = quantidade, color = tipo_da_obra)) +
        ...
        labs(title = "Quantidades de obras agrupadas por tipo ao longo do tempo",
             subtitle = "Considerando obras dos tipos mais construídos a partir de 2003")
```
...
obra.jurisdicionado %>%
    ...
    geom_col() + 
    labs(title = "Jurisdicionado temporalmente", subtitle="Considerando obras a partir de 2003", y = "Porcentagem de obras que possuem nome do jurisdicionado", x= "Ano da obra")
...
No gráfico abaixo podemos ver quais jurisdicionados possuem a maior quantidade de obras (considerando obras de 2003 até os dias atuais). Algumas prefeituras de cidades tidas como pequenas aparecem no ranking enquanto orgãos de cidades maiores como Patos e Souza não aparecem.
